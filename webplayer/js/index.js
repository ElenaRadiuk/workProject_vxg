...
$(document).ready(function() {
	if(vxgcloudplayer.isLocalFile()){
		$('.container').html('<center><br><br><h3>For proper functioning the site must be open from an HTTP-server (for example: nginx, Apache, http-server module for Nodejs etc...) <br> Please install an HTTP-server and publish the site with it.</h3>')
		return;
	}
	...
	$('.mobile-app-download.app-store').unbind().bind('click', function(){
		var url = '';
		if(vxgcloudplayer.isMobile()){
			url = 'https://itunes.apple.com/app/vxg-cloud-player/id1257626446?mt=8';
		}else{
			url = 'https://itunes.apple.com/app/vxg-cloud-player/id1257626446?mt=8';
		}
		var win = window.open(url, '_blank');
	...
	vxgcloudplayer.bind('updated_cameraname_streaming', function(){
		$('.page-link input').val(window.location.href);
		$('#qrcode').html('');
	...
	vxgcloudplayer.bind('updated_current_camid', function(){
		vxgcloudplayer.trigger('updated_cameraname_streaming');
	});

	vxgcloudplayer.bind('camera_for_streaming_online', function(){
		switcher_url = true;
		vxgcloudplayer('vxg_cloud_player1').play();
		$('.pull-video-form').addClass('disabled');
	});
	
	vxgcloudplayer.bind('camera_for_streaming_offline', function(){
		switcher_url = false;
		vxgcloudplayer('vxg_cloud_player1').stop();

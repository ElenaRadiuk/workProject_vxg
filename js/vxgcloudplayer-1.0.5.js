...
vxgcloudplayer.accp_login_req_poling = function(data, d){
	$.ajax({
		...
		if(r.cloud_token && r.cloud_token.token){
			...
			CloudAPI.setURL(response.svcp_host);
			CloudAPI.config.apiToken = {};
			CloudAPI.config.apiToken.token = r.cloud_token.token;
			CloudAPI.config.apiToken.expire = r.cloud_token.expire;
			CloudAPI.config.apiToken.expireTimeUTC = Date.parse(r.cloud_token.expire + "Z");
...
document.addEventListener('DOMContentLoaded', function() {
	...
	if(vxgcloudplayer.isMobile() || !CloudAPI.supportFlash() || CloudAPI.containsPageParam('hls')){
		console.warn("In mobile will be used hls");
		window.vxgcloudplayer.usehls = true;
	}else{
		window.vxgcloudplayer.usehls = CloudAPI.containsPageParam("hls");
	}
	
	if(!CloudAPI.supportFlash() && !vxgcloudplayer.isMobile()){
		$(".comment-flash").show();
	}else{
		$(".comment-flash").hide();
	}
